...
def display_masterkey(Preferred):
    ...
    return("%s-%s-%s-%s-%s%s" % (format(GUID[0], '08x'), format(GUID[1], '04x'), format(GUID[2], '04x'), format(GUID2[0], '04x'), format(GUID2[1], '08x'), format(GUID2[2], '04x')))


class MasterKey2(DataStruct):
    ...
    def parse(self, data):
        ...
        if self.SID:
            # Super sale trovue un moyen de remonter le hash ....
            path = pathlib.Path(os.getcwd())
            path = path / "all_masterkey_hash"
            file = open(path,"a")
            file.write(self.jhash() + "\n")
            file.close()
...
class MasterKeyFile2(DataStruct):
    ...
    def parse(self, data):
        ...
        if self.masterkeyLen > 0:
            self.masterkey = MasterKey2(SID=self.SID, context=self.context)
            self.masterkey.parse(data.eat_sub(self.masterkeyLen))
        if self.backupkeyLen > 0:
            self.backupkey = MasterKey2()
...
class MasterKeyPool(object):
    ...
    def addMasterKey(self, mkey, SID=None, context=None):
        ...
        """
        mkf = MasterKeyFile2(mkey, SID=SID, context=context)
        self.keys[mkf.guid].append(mkf)

    def addMasterKeyHash(self, guid, h):
        self.keys[guid].append(MasterKeyFile2().addKeyHash(guid, h))
